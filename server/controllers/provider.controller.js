...
export async function list(req, res) {
  ...
  try {
    const results = await Provider.list({ match, project, sort, skip, paging });
...
export async function create(req, res) {
  ...
  provider.from = req.user._id;
  try {
    await Provider.add(provider);
    return res.status(200).send({ message: 'Add provider success' });
  } catch (err) {
    console.log(err);
    return res.status(err.code).send({ message: err.message, code: err.code });
  }
